...
esp_err_t max30100_check_sample(uint16_t *ir_data, uint16_t *red_data, size_t data_len);
...
#if DEBUG >= 0
    printf("Setting leds currents\tIR: %x\tRED: %x -> %x\n", IR_LED_CURRENT, RED_LED_CURRENT, RED_LED_CURRENT << 4 | IR_LED_CURRENT);
#endif
...
esp_err_t max30100_read_fifo(uint16_t *ir_data, uint16_t *red_data, size_t *data_len)
{
    uint8_t buffer;
...
esp_err_t max30100_update(uint16_t *ir_data, uint16_t *red_data, size_t *data_len)
{
...
void max30100_adjust_current()
{
    ...
    max30100_write_byte(MAX30100_REG_LED_CONFIGURATION, red_current << 4 | led_conf);
...
void max30100_set_led_width(uint8_t led_pulse_width)
{
    ...
    ret = max30100_write_byte(MAX30100_REG_SPO2_CONFIGURATION, (previous & 0xfc) | led_pulse_width);
...
#endif
    ret = max30100_write_byte(MAX30100_REG_SPO2_CONFIGURATION, (previous & 0xe3) | (sampling_rate << 2));
...
void max30100_set_highres(uint8_t enabled)
{
    ...
    uint8_t mode = enabled ? MAX30100_SPC_SPO2_HI_RES_EN : ~MAX30100_SPC_SPO2_HI_RES_EN;
...
void max30100_test_conf()
{
    ...
    ESP_ERROR_CHECK(max30100_set_leds(MAX30100_LED_CURR_50MA, MAX30100_LED_CURR_50MA));
    printf("done.\n");

    printf("Lowering the current to 7.6mA..\n");
    ESP_ERROR_CHECK(max30100_set_leds(MAX30100_LED_CURR_7_6MA, MAX30100_LED_CURR_7_6MA));
    printf("done.\n");
